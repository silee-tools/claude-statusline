...
# 두 줄과 호출자가 계산한 표시 폭 예산을 넣고 두 줄을 받는다.
fit() { printf '%s\n%s\n' "$1" "$2" | LC_ALL=C awk -v budget="$3" -f "$AWKP"; }
...
# T1: 예산 안이면 그대로 통과한다
R=$(fit "~/↪2/claude-statusline" "main" 66)
...
R=$(fit "~/↪1/webapp/↪2/PROJ-1469-connect-api-gateway" "feature/PROJ-1469-connect-api-secrets" 66)
...
R=$(fit "~/↪1/webapp/↪2/PROJ-1469-connect-api-gateway-extra-long-name-here" "main" 66)
P=$(printf '%s\n' "$R" | sed -n 1p)
assert_equals "T3 브랜치가 짧으면 경로가 62칼럼까지" "62" "$(vwidth "$P")"

# T4: 두 칸 문자를 반으로 쪼개지 않는다
R=$(fit "~/↪2/helper/↪2/성과-여정-2분기-리뷰-아주-긴-이름-테스트-문자열" "성과-여정-2분기-리뷰-아주-긴-브랜치-이름" 66)
P=$(printf '%s\n' "$R" | sed -n 1p); B=$(printf '%s\n' "$R" | sed -n 2p)
...
R=$(fit "$COLORED" "feature/PROJ-1469-connect-api-secrets" 66)
P=$(printf '%s\n' "$R" | sed -n 1p)
...
R=$(fit "~/↪1/webapp/↪2/PROJ-1469-connect-api-gateway-extra-long-name-here-more" "" 68)
P=$(printf '%s\n' "$R" | sed -n 1p)
...
# T9: 브랜치가 있으면 호출자가 계산한 32칼럼 예산을 둘이 나눠 쓴다.
R=$(fit "~/very-long-project-directory-name" "feature/very-long-branch-name" 32)
P=$(printf '%s\n' "$R" | sed -n 1p); B=$(printf '%s\n' "$R" | sed -n 2p)
assert_equals "T9 호출자 예산에서 경로 몫 16칼럼" "16" "$(vwidth "$P")"
assert_equals "T9 호출자 예산에서 브랜치 몫 16칼럼" "16" "$(vwidth "$B")"
assert_equals "T9 호출자 예산으로 첫 행 40칼럼" "40" "$(line1_width "$P" "$B")"

# T10: 브랜치가 없으면 호출자가 계산한 34칼럼 예산을 경로가 모두 쓴다.
R=$(fit "~/very-long-project-directory-name-that-overflows" "" 34)
P=$(printf '%s\n' "$R" | sed -n 1p)
assert_equals "T10 브랜치 부재 시 호출자 예산 34칼럼" "34" "$(vwidth "$P")"
assert_equals "T10 호출자 예산으로 첫 행 40칼럼" "40" "$(line1_width "$P" "")"
