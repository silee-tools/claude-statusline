...
assert_equals "T1 세션별 카운터를 1 로 기록" "1" "$(counter_value "$SESSION")"

# T2: 같은 세션의 재발화는 카운터를 누적한다.
run "$INPUT"
assert_equals "T2 같은 세션의 두 번째 발화도 재개" "2" "$rc"
assert_equals "T2 카운터 누적" "2" "$(counter_value "$SESSION")"
...
assert_equals "T4 다른 세션의 카운터는 1" "1" "$(counter_value "$OTHER")"
CLAUDE_RESUME_MAX_ATTEMPTS=72
...
assert_equals "T5 session_id 없으면 unknown 카운터 사용" "1" "$(counter_value unknown)"
...
assert_equals "T6 비정상 session_id 는 unknown 으로 격리" "1" "$(counter_value unknown)"
ENTRIES=$(ls -A "$CLAUDE_RESUME_STATE_DIR" | tr '\n' ' ')
assert_equals "T6 상태 디렉터리에는 unknown 만 존재" "unknown " "$ENTRIES"
